...
<body class="ecommerce">
            ...
            <table class="table table-striped table-bordered table-hover" id="userList" style=" text-align:center; ">
									...
									<thead>
										<tr>
											...
											<th width="70%" style="text-align:center;">商品种类</th>
									...
									<tbody>
										<c:forEach items="${ctgList}" var="ctg" varStatus="status" >										
											<%-- <input type="text" id="cname" value="${ctg.cname}" style="display: none"> --%>
											<tr class="odd gradeX">
												<td>${status.count} </td>
												<td>${ctg.cname}</td>
												<td><button style="background: none;border-color:black"  onclick="ctg('del','${ctg.cname}')">删除</button>
												<button style="background: none;border-color:black"  onclick="ctg('edt','${ctg.cname}')">编辑</button>
												<button style="background: none;border-color:black"  onclick="ctg('add','${ctg.cname}')">增加</button></td>
    ...
    <script type="text/javascript">
        ...
        function ctg(data,name){
        	...
        	if(data=="del"){
        		...
        		$.ajax({
                    ...
                    success: function(result) {
     			    	...
     			    	if(result =="success") {
     			    		alert("删除成功")
     			    	   location.href = "/chanyan/manager/toCategory"; 
     			      }else{
     			    	   alert("删除失败");
     			      }
     			    },
        	...
        	if(data=="edt"){
        		newName = prompt("请输入新的类别名");
        		if(newName){ 
        			//点击的是“确定” 
        			$.ajax({
                        type:'post',
                        url:'/chanyan/manager/edtCtg',
                        dataType:"text",//注意使用的是打他dataType，而不是Content-Type
                        data:{cname:name,newName:newName},
                        success: function(result) {
         			    	 console.log(result);
         			    	if(result =="success") {
         			    		alert("修改成功")
        	...
        	if(data=="add"){
                 newName = prompt("请输入类别名");
        		if(newName){ 
        			//点击的是“确定” 
        			$.ajax({
                        type:'post',
                        url:'/chanyan/manager/addCtg',
                        dataType:"text",//注意使用的是打他dataType，而不是Content-Type
                        data:{cname:newName},
                        success: function(result) {
         			    	 console.log(result);
